...
// jQuery(function ($) {
    ...
    // //navButtons 自定义按钮
    // jQuery(casetype_grid).jqGrid('navGrid', casetype_pager, {
    //     edit: false,
    //     add: false,
    //     del: false,
    //     search: false,
...
function addCheckLeaderCSS(obj) {
    ...
    if (hasboss == true) {  //前面是默认上级,那么后面只能是指定人选了
        ...
        $(obj).before('<span class="morecheckleader"><span class="glyphicon glyphicon-arrow-right"></span><select class="select2 checkleader" id="checkleader_' + len + '" onchange="changeCheckLeader(this);" style="width:100px" num="0"><option value = "-2">指定人选</option></select></span>');
        getCheckLeader($("#checkleader_" + len));
    } else {
        $(obj).before('<span class="morecheckleader"><span class="glyphicon glyphicon-arrow-right"></span><select class="select2 checkleader" onchange="changeCheckLeader(this);" style="width: 100px"> <option value="-1">默认上级</option><option value = "-2">指定人选</option></select></span>')
    }
    if ($(obj).next().is("a")) {
    } else {
...
$('#modalCaseTypeOperating').on('hide.bs.modal', function (e) {
    ...
    $("#addcheckleader").before('<span class="firstcheckleader"><select class="select2 checkleader" name="firstcheckleader" id="firstcheckleader" style="width:100px" onchange="changeCheckLeader(this);"><option value="-1">默认上级</option><option value = "-2">指定人选</option></select></span>');
})
